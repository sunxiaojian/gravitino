...
public class FileSystemUtils {
  ...
  public static Configuration createConfiguration(String bypass, Map<String, String> config) {
    try (ByteArrayOutputStream out = new ByteArrayOutputStream()) {
      ...
      config.forEach(
          (k, v) -> writeProperty(writer, bypass != null ? k.replace(bypass, "") : k, v));
  ...
  private static Configuration loadConfigFromStream(ByteArrayOutputStream out) {
    Configuration conf = new Configuration(false);
    conf.addResource(new ByteArrayInputStream(out.toByteArray()));
    return conf;
  }
}
